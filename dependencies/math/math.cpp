...
bool math::world_to_screen_vec3(const vec3_t& origin, vec3_t& screen) {
	auto matrix = interfaces::engine->world_to_screen_matrix();
...
void math::angle_vectors_const(const vec3_t& angles, vec3_t& forward) {
	float	sp, sy, cp, cy;
...
float math::get_fov(const vec3_t& view_angle, const vec3_t& aim_angle) {
	...
	angle_vectors_const(view_angle, aim);
	angle_vectors_const(aim_angle, ang);
